...
platformV28Routes.get('/v28/driver/calls/:id',async c=>{
  ...
  if(validPoint(Number(driver?.current_lat),Number(driver?.current_lng))&&validPoint(Number(item.pickup_lat),Number(item.pickup_lng)))distanceToPickup=distanceMeters(Number(driver.current_lat),Number(driver.current_lng),Number(item.pickup_lat),Number(item.pickup_lng));
