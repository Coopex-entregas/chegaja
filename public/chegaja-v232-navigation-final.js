/* ChegaJá 14.33.6 — autorização GPS no clique, online persistente e SOS resiliente */
(()=>{
'use strict';
...
const R={last:null,lastAt:0,sosBusy:false,onlineBusy:false,toggleBusy:false,online:null,active:null};
...
async function serverLocation(){
 try{
  const data=await request('/api/app/driver/live',{timeout:5500}),d=data.driver||{};
  R.online=Boolean(Number(d.online||0));R.active=data.active||null;
...
function paintOnline(){
 ...
 let online=R.online;
 if(online==null){const saved=localStorage.getItem('cj_driver_online');if(saved==='1'||saved==='0')online=saved==='1';else if(window.state?.user?.online!=null)online=Boolean(Number(window.state.user.online))}
 if(online==null)return;
 const active=Boolean(activeDelivery());
...
async function toggleOnline(event){
 ...
 try{
  ...
  const active=Boolean(activeDelivery());
  if(R.online&&active){toastMessage('Há uma entrega em andamento. Você permanecerá online até finalizar.');return}
  if(R.online){
   ...
   toastMessage('Você ficou offline.');
  }else{
   toastMessage('Autorize a localização do celular para continuar.');
   ...
   toastMessage('Você está online. Localização autorizada.');
  }
...
setInterval(tick,800);setInterval(()=>syncOnline(false),7000);
