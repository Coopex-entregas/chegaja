/* ChegaJá 14.19.2 — mapa estável com Google validado e contingência */
(()=>{
'use strict';
...
async function config(force=false){
  ...
  if(!response.ok||data.ok===false)throw new Error(data.error||'Não foi possível ler a configuração do mapa.');
...
async function ensureGoogle(){
  ...
  runtime.google=(async()=>{
    ...
    // Uma chave de servidor não deve ser usada no navegador. Ela normalmente é
    // bloqueada por restrição de IP e deixa o mapa cinza. Nesse caso usamos a
    // contingência até o Master cadastrar a chave própria do navegador.
    if(cfg.provider!=='google'||!cfg.enabled||!cfg.api_key||cfg.key_source!=='browser')throw new Error('Chave Google de navegador não configurada.');
    const existing=document.querySelector('script[data-chegaja-google-maps],script[src*="maps.googleapis.com/maps/api/js"]');
    if(existing){
      await new Promise((resolve,reject)=>{const started=Date.now();const check=()=>window.google?.maps?.Map?resolve():Date.now()-started>8000?reject(new Error('O Google Maps não terminou de carregar.')):setTimeout(check,100);check()});
      return cfg;
    }
    await new Promise((resolve,reject)=>{
      const callback=`__chegajaGoogle${Date.now()}`;
      const timeout=setTimeout(()=>{delete window[callback];reject(new Error('Tempo esgotado ao carregar o Google Maps.'))},8000);
      ...
      script.onerror=()=>{clearTimeout(timeout);delete window[callback];reject(new Error('Google Maps bloqueado para este domínio.'))};
...
function geometryPoints(raw){
  ...
  return value.map(item=>{
    if(Array.isArray(item)){
      const a=Number(item[0]),b=Number(item[1]),asLatLng=point(a,b),asGeo=point(b,a);
...
function photoHtml(url,label='CJ'){
  return url?`<div class="cj191-photo-marker"><img src="${escapeHtml(url)}" alt=""><i></i></div>`:`<div class="cj191-letter-marker">${escapeHtml(label).slice(0,2)}</div>`;
}
function createLeafletMap(host,options={}){
  if(!window.L)throw new Error('Motor de mapa indisponível.');
  host.replaceChildren();
  host.classList.add('cj191-leaflet-host','gm-style');
  const center=point(options.center||[-5.7945,-35.211]);
  const map=L.map(host,{zoomControl:true,attributionControl:true,preferCanvas:true}).setView([center.lat,center.lng],Number(options.zoom||13));
  ...
  const adapter={
    provider:'contingency',raw:map,host,
    ...
    addMarker(value,opt={}){
      ...
      const icon=L.divIcon({className:'cj191-leaflet-icon',html:photoHtml(opt.photo,opt.label||'CJ'),iconSize:[38,44],iconAnchor:[19,40]});
      const marker=L.marker([p.lat,p.lng],{icon,title:opt.title||''}).addTo(map);
      if(opt.popup)marker.bindPopup(opt.popup);
      return remember(opt.group,marker)
    },
    addCircleMarker(value,opt={}){return adapter.addMarker(value,opt)},
    addPolyline(values,opt={}){const path=(values||[]).map(item=>point(item)).filter(valid).map(p=>[p.lat,p.lng]);if(!path.length)return null;return remember(opt.group,L.polyline(path,{color:opt.color||'#0D257A',weight:Number(opt.weight||6),opacity:Number(opt.opacity??.9)}).addTo(map))},
    ...
    panTo(value){const p=point(value);if(valid(p))map.panTo([p.lat,p.lng])},
    fitBounds(values,opt={}){const pts=(values||[]).map(item=>point(item)).filter(valid).map(p=>[p.lat,p.lng]);if(pts.length)map.fitBounds(pts,{padding:[Number(opt.padding||35),Number(opt.padding||35)],maxZoom:Number(opt.maxZoom||17)})},
    on(event,handler){map.on(event,handler)},
    invalidateSize(){setTimeout(()=>map.invalidateSize(false),0)},resize(){setTimeout(()=>map.invalidateSize(false),0)},
    remove(){for(const key of [...groups.keys()])adapter.clearGroup(key);try{map.remove()}catch{}runtime.instances.delete(host.id||host);host.replaceChildren()}
  };
...
async function createGoogleMap(host,options={}){
  await ensureGoogle();
  const center=point(options.center||[-5.7945,-35.211]);
  host.replaceChildren();host.classList.remove('cj191-leaflet-host');host.classList.add('cj180-google-host');
  ...
  const remember=(group,item)=>{const key=group||'default';if(!groups.has(key))groups.set(key,[]);groups.get(key).push(item);return item};
  const wrap=(raw,position,group)=>{const item={raw,setLatLng(value){const p=point(value);if(valid(p)){position=p;raw instanceof PhotoOverlay?raw.setPosition(p):raw.setPosition(p)}return item},getLatLng(){return[position.lat,position.lng]},bindPopup(html){const open=()=>{info.setContent(String(html||''));info.setPosition(position);info.open({map})};raw instanceof PhotoOverlay?raw.node.addEventListener('click',open):raw.addListener('click',open);return item},remove(){raw.setMap(null)}};return remember(group,item)};
  const adapter={provider:'google',raw:map,host,
    ...
    addMarker(value,opt={}){const p=point(value);if(!valid(p))return null;const photo=opt.photo||(String(opt.label||'')==='EU'?window.ChegaJaCurrentDriverPhoto:null)||window.ChegaJaDriverPhotosByName?.[String(opt.title||'')];const raw=photo?new PhotoOverlay(p,photoNode(photo,opt.label||'CJ')):new google.maps.Marker({map,position:p,title:opt.title||'',icon:{url:svgPin(opt.color||'#0D257A',opt.label||''),scaledSize:new google.maps.Size(42,52),anchor:new google.maps.Point(21,50)}});const item=wrap(raw,p,opt.group);if(opt.popup)item.bindPopup(opt.popup);return item},
    addCircleMarker(value,opt={}){return adapter.addMarker(value,opt)},
    addPolyline(values,opt={}){const path=(values||[]).map(item=>point(item)).filter(valid);if(!path.length)return null;const raw=new google.maps.Polyline({map,path,strokeColor:opt.color||'#0D257A',strokeWeight:Number(opt.weight||6),strokeOpacity:Number(opt.opacity??.9)});return remember(opt.group,{raw,remove(){raw.setMap(null)}})},
    addGeoJSON(raw,opt={}){return adapter.addPolyline(geometryPoints(raw),opt)},
    setView(value,zoom){const p=point(value);if(valid(p))map.setCenter(p);if(zoom!=null)map.setZoom(Number(zoom))},panTo(value){const p=point(value);if(valid(p))map.panTo(p)},
    ...
    on(event,handler){return google.maps.event.addListener(map,event,handler)},invalidateSize(){google.maps.event.trigger(map,'resize')},resize(){google.maps.event.trigger(map,'resize')},
...
async function createMap(hostOrId,options={}){
  ...
  try{return await createGoogleMap(host,options)}catch{return createLeafletMap(host,options)}
}
