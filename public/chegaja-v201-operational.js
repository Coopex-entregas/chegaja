/* ChegaJá 14.28.0 — Leaflet visível; Base sem traçado de rota */
(()=>{
'use strict';
if(window.__cj201Operational14280)return;window.__cj201Operational14280=true;
...
function createLeaflet(host,options={}){
 ...
 const baseMap=host.id==='v31-base-map'||Boolean(host.closest?.('.v31-base-shell'));
 const adapter={provider:'leaflet',raw:map,host,
  ...
  addPolyline(values,opt={}){if(baseMap)return null;const path=(values||[]).map(item=>point(item)).filter(valid).map(p=>[p.lat,p.lng]);if(!path.length)return null;return remember(opt.group,L.polyline(path,{color:opt.color||'#0d45d8',weight:Number(opt.weight||6),opacity:Number(opt.opacity??.85)}).addTo(map))},
  addGeoJSON(raw,opt={}){if(baseMap)return null;return adapter.addPolyline(geometry(raw),opt)},
...
function forceMapEngine(){if(!window.ChegaJaMaps)return;window.ChegaJaMaps.createMap=async(hostOrId,options={})=>{const host=typeof hostOrId==='string'?document.getElementById(hostOrId):hostOrId;if(!host)throw new Error('Área do mapa não encontrada.');return createLeaflet(host,options)}}
function removeSoundButtons(){$$('button').forEach(button=>{if(/ativar sons|sons ativos/i.test(button.textContent||''))button.remove()})}
async function api(path){const auth=String(window.state?.token||localStorage.getItem('lg_token')||'');const response=await fetch(path,{headers:auth?{Authorization:`Bearer ${auth}`}:{},cache:'no-store'});const data=await response.json().catch(()=>({}));if(!response.ok||data.ok===false)throw new Error(data.error||`Erro ${response.status}`);return data}
function ensureEstablishmentLeaflet(){const source=$('#cj14-est-map');if(!source||window.state?.user?.role!=='establishment'||window.state?.page!=='dashboard'){destroyEst();return}const parent=source.parentElement;if(!parent)return;parent.classList.add('cj201-map-parent');let host=$('.cj201-est-map',parent);if(!host){host=document.createElement('div');host.className='cj201-est-map';parent.appendChild(host)}source.style.visibility='hidden';if(runtime.estHost!==host){destroyEst();runtime.estHost=host;runtime.est=createLeaflet(host,{center:[-5.7945,-35.211],zoom:13,zoomControl:true});runtime.estLayer=L.layerGroup().addTo(runtime.est.raw)}refreshEstablishment()}
async function refreshEstablishment(){if(!runtime.est||runtime.est.busy)return;runtime.est.busy=true;try{const [drivers,deliveries]=await Promise.all([api('/api/app/tenant/online-drivers'),api('/api/app/tenant/deliveries')]);runtime.estLayer.clearLayers();const points=[];for(const item of drivers.items||[]){if(Number(item.online)!==1)continue;const p=point(item.current_lat,item.current_lng);if(!valid(p))continue;points.push([p.lat,p.lng]);L.marker([p.lat,p.lng],{icon:photoIcon(item.photo_url,item.name,32)}).addTo(runtime.estLayer).bindPopup(`<strong>${esc(item.name||'Cooperado')}</strong><br>${esc(item.vehicle_plate||'Online')}`)}for(const item of deliveries.items||[]){if(['delivered','cancelled'].includes(String(item.status)))continue;for(const [lat,lng,label,color] of [[item.pickup_lat,item.pickup_lng,'Coleta','#16a05e'],[item.delivery_lat,item.delivery_lng,'Entrega','#ff643f']]){const p=point(lat,lng);if(!valid(p))continue;points.push([p.lat,p.lng]);L.circleMarker([p.lat,p.lng],{radius:7,weight:3,color,fillColor:color,fillOpacity:1}).addTo(runtime.estLayer).bindPopup(`<strong>${esc(item.display_code||'Entrega')}</strong><br>${label}`)}}if(!runtime.est.fitted&&points.length){runtime.est.raw.fitBounds(points,{padding:[35,35],maxZoom:15,animate:false});runtime.est.fitted=true}runtime.est.raw.invalidateSize(false)}catch{}finally{runtime.est.busy=false}}
