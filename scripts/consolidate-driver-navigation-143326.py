...
def replace_line(s,prefix,new,label):
    lines=s.splitlines()
...
def insert_before(s,prefix,block,label):
    lines=s.splitlines()
    for i,line in enumerate(lines):
        if line.startswith(prefix):
            lines[i:i]=block.splitlines()
            return '\n'.join(lines)+('\n' if s.endswith('\n') else '')
    raise RuntimeError(f'Ponto não encontrado: {label}')

js_path='public/chegaja-v217-driver-navigation.js'
js=read(js_path)
if 'ChegaJá 14.33.26' not in js:
    js=js.replace('/* ChegaJá 14.33.25 — rota e posição em camadas Leaflet padrão */','/* ChegaJá 14.33.26 — navegação heading-up com bússola, seta e manobras */',1)
    js=js.replace('__CJ_DRIVER_LEAFLET_143325__','__CJ_DRIVER_LEAFLET_143326__')
    js=replace_once(js,'voicePtBr:null};',"voicePtBr:null,headingUp:true,deviceHeading:null,lastSpeed:0,orientationBound:false,routeArrows:[],maneuverMarkers:[],navSteps:[],nextInstruction:null};",'estado')
    js=replace_once(js,'<button id="cj199-center" type="button" aria-label="Centralizar na minha localização">◎</button>','<button id="cj199-center" type="button" aria-label="Centralizar na minha localização">◎</button><button id="cj217-bearing" type="button" aria-label="Alternar orientação do mapa"><span>➤</span><small>AUTO</small></button>','botão orientação')
...
function refreshGuidance(){drawRouteArrows();drawManeuvers(A.navSteps);refreshRouteArrowAngles()}'''
    js=insert_before(js,'function smoothHeading(',helpers,'helpers')

    js=replace_line(js,'function paintHeading(',"function paintHeading(){const el=A.self?.getElement?.()?.querySelector('.cj217-self-marker');if(!el)return;const h=effectiveHeading(),mapBearing=currentMapBearing(),angle=h==null?0:normAngle(h-mapBearing);el.style.transform=`rotate(${angle||0}deg)`}",'paint heading')
    js=replace_line(js,'function ensureSelf(',"function ensureSelf(){if(!A.map||!valid(A.gps))return;const ll=[A.gps.lat,A.gps.lng],markerLike=A.self&&typeof A.self.setIcon==='function';if(!markerLike){try{A.self?.remove()}catch{}A.self=L.marker(ll,{icon:selfIcon(),keyboard:false,interactive:false,zIndexOffset:9000,title:'Sua localização'}).addTo(A.map)}else A.self.setLatLng(ll);paintHeading()}",'self marker')

    lines=js.splitlines()
    for i,line in enumerate(lines):
        if line.startswith('async function ensureMap('):
            old="A.map=L.map(host,{zoomControl:true,attributionControl:true,preferCanvas:false,zoomSnap:.5,zoomDelta:.5,fadeAnimation:false,markerZoomAnimation:false}).setView([center.lat,center.lng],valid(A.gps)?18:13);"
            new="A.map=L.map(host,{zoomControl:true,attributionControl:true,preferCanvas:false,zoomSnap:.5,zoomDelta:.5,fadeAnimation:false,markerZoomAnimation:false,rotate:typeof L.Map?.prototype?.setBearing==='function',bearing:0,touchRotate:true,rotateControl:false}).setView([center.lat,center.lng],valid(A.gps)?18:13);"
            if old not in line: raise RuntimeError('ensureMap base não encontrado')
            lines[i]=line.replace(old,new,1)
            break
    else: raise RuntimeError('ensureMap não encontrado')
    js='\n'.join(lines)+('\n' if js.endswith('\n') else '')

    js=replace_line(js,'function drawRoute(',"function drawRoute(points){if(!A.map||!Array.isArray(points)||points.length<2)return;const path=points.map(p=>point(p.lat,p.lng)).filter(valid);if(path.length<2)return;A.routePoints=path;const ll=path.map(p=>[p.lat,p.lng]);if(!A.casing)A.casing=L.polyline(ll,{color:'#fff',weight:14,opacity:.98,lineCap:'round',lineJoin:'round',interactive:false,className:'cj217-route-casing'}).addTo(A.map);else A.casing.setLatLngs(ll);if(!A.line)A.line=L.polyline(ll,{color:'#075dff',weight:8,opacity:1,lineCap:'round',lineJoin:'round',interactive:false,className:'cj217-route-line'}).addTo(A.map);else A.line.setLatLngs(ll);A.casing.bringToFront?.();A.line.bringToFront?.();ensureSelf();requestAnimationFrame(()=>{try{A.casing?.redraw?.();A.line?.redraw?.();refreshGuidance()}catch{}})}",'draw route')
    js=replace_line(js,'function clearRoute(',"function clearRoute(){A.casing?.remove();A.line?.remove();A.casing=A.line=null;A.routePoints=[];clearGuidance();A.lastRouteOrigin=A.lastRouteTarget=null;A.routeFitKey='';A.navTarget=null;A.navFrameAt=0;A.navFrameOrigin=A.navFrameTarget=null}",'clear route')
    js=replace_line(js,'function frameNavigation(',"function frameNavigation(force=false){if(!A.map||!valid(A.gps)||!navigationActive()||A.manualView||!A.following)return false;const target=targetPoint();if(!valid(target))return false;const moved=!valid(A.navFrameOrigin)||distance(A.gps,A.navFrameOrigin)>18,targetChanged=!valid(A.navFrameTarget)||distance(target,A.navFrameTarget)>15;if(!force&&!moved&&!targetChanged&&Date.now()-A.navFrameAt<3500){applyMapBearing(false);return false}const d=distance(A.gps,target),zoom=d<180?18.5:d<500?17.8:d<1400?17.2:16.6;A.programmatic=true;try{A.map.setView([A.gps.lat,A.gps.lng],zoom,{animate:false});applyMapBearing(force);A.navFrameAt=Date.now();A.navFrameOrigin={...A.gps};A.navFrameTarget={...target};ensureSelf();updateStops();refreshGuidance()}finally{setTimeout(()=>A.programmatic=false,120)}return true}",'frame navigation')
    ...
    lines=js.splitlines()
    for i,line in enumerate(lines):
        if line.startswith('function applyMetric('):
            old="else if(A.detail){label.textContent='NAVEGAÇÃO ATIVA';value.textContent=statusText(A.detail.status).toUpperCase();hint.textContent='Siga a linha azul; o mapa acompanha sua posição em tempo real'}"
            new="else if(A.detail){const nav=A.nextInstruction;label.textContent='NAVEGAÇÃO ATIVA';value.textContent=nav?.text?`${nav.glyph||'↑'} ${nav.text}`:statusText(A.detail.status).toUpperCase();hint.textContent=nav&&Number(nav.meters)>0?`${Math.round(nav.meters)} m • ${targetKind()==='pickup'?'indo para a coleta':'indo para a entrega'}`:'Siga a linha azul; o sentido da marcha fica para cima'}"
            if old not in line: raise RuntimeError('applyMetric bloco não encontrado')
            lines[i]=line.replace(old,new,1)
            break
    else: raise RuntimeError('applyMetric não encontrado')
    js='\n'.join(lines)+('\n' if js.endswith('\n') else '')

    js=replace_line(js,'function handleGpsPosition(',"function handleGpsPosition(p){const next=point(p.coords.latitude,p.coords.longitude),moved=valid(A.lastGps)?distance(A.lastGps,next):Infinity;A.gps=next;A.gpsError=null;A.lastSpeed=Number(p.coords.speed||0);A.lastHeading=smoothHeading(p.coords.heading,p.coords.speed,p.coords.accuracy);window.ChegaJaLastDriverLocation={lat:next.lat,lng:next.lng,accuracy:p.coords.accuracy,heading:A.lastHeading,speed:p.coords.speed};ensureSelf();trimRouteToGps();applyMapBearing(false);if(A.map&&!A.firstCenter){A.firstCenter=true;centralize()}else followGps();const loc=positionPayload(p),moving=A.lastSpeed>.8||moved>10,interval=A.detail?(moving?4000:6500):(moving?7000:12000);if(A.online&&Date.now()-A.lastSent>=interval){A.lastGps={...next};pushLocation(loc).catch(()=>{});autoProgress(loc)}if(A.detail&&moving)updateRoute(false)}",'gps')

    lines=js.splitlines()
    for i,line in enumerate(lines):
        if line.startswith('async function toggleOnline('): lines[i]=line.replace('async function toggleOnline(){unlockAudio();','async function toggleOnline(){requestOrientationPermission().catch(()=>{});unlockAudio();',1)
        elif line.startswith('function bind('): lines[i]=line.replace("$('#cj199-center').onclick=centralize;","$('#cj199-center').onclick=centralize;$('#cj217-bearing').onclick=toggleBearingMode;installOrientationListeners();",1)
        elif line.startswith('async function mount('): lines[i]=line.replace('installAudioArm();startGps();primeAlarmMedia(false);','installAudioArm();installOrientationListeners();startGps();primeAlarmMedia(false);',1)
    js='\n'.join(lines)+('\n' if js.endswith('\n') else '')
    write(js_path,js)
...
#cj217-bearing span{font-size:22px;line-height:18px;transform:rotate(-90deg)}#cj217-bearing small{font-size:7px;line-height:10px;letter-spacing:.08em}#cj217-bearing.north span{color:#667085}#cj217-bearing.north small{color:#667085}
...
leaflet='<script src="/vendor/leaflet/leaflet.js?recovery=143314"></script>'
if 'leaflet-rotate@0.2.8' not in index:
    index=index.replace(leaflet,leaflet+'<script src="https://unpkg.com/leaflet-rotate@0.2.8/dist/leaflet-rotate.js" crossorigin="anonymous"></script>',1)
...
# app.js continua em 14.33.25; só o painel de navegação mudou nesta versão.
test=test.replace('app\\.js\\?v=14\\.33\\.26&recovery=143326','app\\.js\\?v=14\\.33\\.25&recovery=143325')
...
print('ChegaJá 14.33.26 consolidado: mapa heading-up, seta do cooperado, setas/manobras, voz pt-BR e mapa limpo.')
